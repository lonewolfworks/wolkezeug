 ...
 */
package com.lonewolfworks.wolke.aws.ecs.broker.sqs;
...
public class SqsBroker {
    ...
    public void brokerQueue(AmazonSQS client, SqsQueue queue, String queuePolicy, List<HermanTag> tags) {
        ...
        String dlqSuffix = StringUtils.isNotEmpty(queue.getDlqSuffix())?queue.getDlqSuffix():"-dlq";
        String dlqName = queue.getName() + dlqSuffix;
        ...
        if (queuePolicy != null) {
            ...
            dlqAttributes.put("Policy", fullPolicy);
        }
    ...
    private void createNewQueue(AmazonSQS client, String queueName, Map<String, String> attributes) {
        buildLogger.addLogEntry("Could not find queue with name: " + queueName + ". Attempting to create...");
        ...
        try {
            ...
            buildLogger.addErrorLogEntry(String.format("Error creating queue %s: %s", request.getQueueName(), request.getAttributes()));
            buildLogger.addErrorLogEntry(String.format("With Exception: %s", createQueueEx.getMessage()));
            throw createQueueEx;
        }
    }
    ...
    private void updateQueueAttributes(AmazonSQS client, String queueName, Map<String, String> attributes) {
        ...
        try {
            ...
            buildLogger.addErrorLogEntry(String.format("Error updating attributes for queue %s: %s", queueName, attributes));
            throw sqaEx;
        }
    }
