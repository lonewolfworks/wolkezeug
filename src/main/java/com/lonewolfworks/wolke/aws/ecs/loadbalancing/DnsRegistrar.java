...
public class DnsRegistrar {
    ...
    public void registerDns(String elbName, String elbType, String protocol, String registeredUrl, boolean internetFacingUrlScheme) {
    	if(StringUtils.isEmpty(dnsLambda)) {
    		buildLogger.addLogEntry("... DNS Broker disabled...");
    		return;
    	}
       	...
       	DnsBrokerRequest dnsBrokerRequest = new DnsBrokerRequest()
                ...
                .withInternetFacingUrlScheme(internetFacingUrlScheme);
        ...
        if (isSuccessful(invokeResult.getStatusCode()) && StringUtils.isEmpty(invokeResult.getFunctionError())) {
            String dnsBrokerUpdatesJson = new String(invokeResult.getPayload().array(), StandardCharsets.UTF_8);
            List<String> updates;
            try {
                updates = new ObjectMapper().readValue(dnsBrokerUpdatesJson, new TypeReference<List<String>>() {
                });
            } catch (Exception e) {
                throw new RuntimeException("Unable to parse broker updates from: " + dnsBrokerUpdatesJson, e);
            }

            updates.forEach(update -> buildLogger.addLogEntry(String.format("... DNS Broker: %s", update)));
        } else {
            buildLogger.addLogEntry("... Error thrown by the DNS Broker given payload: " + dnsBrokerRequest);
            String brokerResponseJson = new String(invokeResult.getPayload().array(), StandardCharsets.UTF_8);
