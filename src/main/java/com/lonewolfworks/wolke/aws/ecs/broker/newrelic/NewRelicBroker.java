...
public class NewRelicBroker {
    ...
    public void brokerNewRelicApplicationDeployment(
    	...
    	if(StringUtils.isEmpty(brokerConfiguration.getBrokerProperties().getNrLambda())){
            buildLogger.addLogEntry("\n");
            buildLogger.addLogEntry("New Relic alert broker disabled...");
            return;
    	}
        ...
        if(newRelicApplicationName!=null) {
        	propertyHandler.addProperty("newrelic.appName", newRelicApplicationName);
        }
        String payload;
        try {
    ...
    private NewRelicConfiguration getNewRelicConfiguration(NewRelicConfiguration newRelicConfigurationDefinition) {
        ...
        if (newRelicConfigurationDefinition != null) {
            ...
            String pluginConditions = null;
            if (StringUtils.isNotBlank(newRelicConfigurationDefinition.getPluginConditions())) {
                pluginConditions = propertyHandler.mapInProperties(
                    fileUtil.findFile(newRelicConfigurationDefinition.getPluginConditions(), false));
            }
